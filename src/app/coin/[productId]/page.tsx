...
import { PredictiveSearch } from '@/components/core/PredictiveSearch/PredictiveSearch';
...
export default function Coin() {
  ...
  const [duration, setDuration] = useState<'1' | '7' | '30' | '365' | 'max'>(
    '1'
  );
    ...
    const fetchCoinData = async () => {
      try {
        const res = await fetch(
          `https://coingeko.burjx.com/coin-ohlc?productId=${coin.productId}&days=${duration}`
        );
  ...
  if (pageLoading)
    return <Loader loadingMessage="Loading coin data" type="page" />;
    ...
    <div className="main">
      <div className="coinDetails">
        <Button
          ...
          ariaLabel={`${coin.name}- click to change coin`}
          type="secondary"
      ...
      <div className="cardContainer">
        <Card width="inherit">
          <div className="flex items-start">
            ...
            <DualSwitch
                label="switch between candlestick and line chart"
              options={[
                {
                  label: "line",
                  icon: {
              ...
              onChange={(obj) =>
                setChartType(obj.label as "line" | "candlestick")
              }
            />
          </div>
          <div className="w-full h-[350px] relative">
            {loading ? (
              <Loader loadingMessage="Loading coin data" type="section" />
            ) : (
              <Chart type={chartType} data={timeline} />
            )}
          </div>
          <div className="w-full flex items-center mt-[60px]">
            <RadioBtnGroup
              ...
              onChange={(obj) =>
                setDuration(obj.value as "1" | "7" | "30" | "365" | "max")
              }
      ...
      <Modal
        ...
        title="Search crypto"
      >
        <PredictiveSearch coins={coinList} onSelect={(value) => setSearchTerm(value)}/>
